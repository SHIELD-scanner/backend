...
class TestAPIIntegration:
    ...
    def mock_mongo_setup(self):
        """Setup mock MongoDB for integration tests."""
    ...
    @patch('app.core.databaseClient.DatabaseClient.client', new_callable=lambda: mongomock.MongoClient())
    def test_exposed_secrets_integration(self, mock_client, client):
        """Integration test for exposed secrets endpoints."""
        # This test verifies the full request/response cycle
        # Note: We're mocking at the database level to avoid real MongoDB dependency
    ...
    @patch('app.core.databaseClient.DatabaseClient.client', new_callable=lambda: mongomock.MongoClient())
    def test_sbom_integration(self, mock_client, client):
    ...
    @patch('app.core.databaseClient.DatabaseClient.client', new_callable=lambda: mongomock.MongoClient())
    def test_vulnerabilities_integration(self, mock_client, client):
    ...
    def test_dependency_injection_consistency(self, client):
        ...
        for endpoint in endpoints_with_dependencies:
            ...
            assert response.status_code != 422  # Unprocessable Entity (dependency issues)
